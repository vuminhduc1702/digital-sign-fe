...
import { useCreateDevRole, type CreateDevRoleDTO } from '../api'
...
export function CreateDevRole({ projectId }: any) {
  const { t } = useTranslation()
  const [type, setType] = useState('Generic')

  const roleType = ['Generic', 'Group']

  const { mutate, isLoading, isSuccess } = useCreateDevRole()
  ...
  return (
    <FormDrawer
      ...
      title="Tạo mới dev role"
      submitButton={
        <Button
          className="rounded border-none"
          form="create-dev-role"
          type="submit"
          size="lg"
          isLoading={isLoading}
          startIcon={
      ...
      size="lg"
    >
      <FormMultipleFields<CreateDevRoleDTO['data'], typeof roleSchema>
        id="create-dev-role"
        onSubmit={values => {
