...
import { actionsList } from './CreateDevRole'
import { useDeleteDevRole } from '../api'
import { UpdateDevRole } from './UpdateDevRole'
...
function RoleTableContextMenu({
  ...
  const { mutate, isLoading, isSuccess } = useDeleteDevRole()
...
export function DevRoleTable({ data, ...props }: RoleTableProps) {
  const { t } = useTranslation()
