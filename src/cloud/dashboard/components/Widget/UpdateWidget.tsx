...
import { type Widget, type WidgetCreate, widgetCreateSchema, type widgetSchema, widgetDataTypeOptions, wsInterval, widgetAgg } from './CreateWidget'
...
export function UpdateWidget({
  ...
  console.log(widgetInfo, 'widgetInfowidgetInfowidgetInfowidgetInfo')

  const initParse = widgetInfo?.datasource?.init_message &&  JSON.parse(widgetInfo?.datasource?.init_message)

  console.log(initParse, 'initParseinitParseinitParseinitParse')
  ...
  useEffect(() => {
    if(initParse?.entityDataCmds[0]?.query?.entityFilter?.entityIds) {
      attrChartMutate({
        data: {
          entity_ids: initParse?.entityDataCmds[0]?.query?.entityFilter?.entityIds,
          entity_type: 'DEVICE',
          // time_series: true,
        },
      })
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 gap-x-4 px-2 md:grid-cols-3">
                    ...
                    <div className="space-y-1">
                      <SelectDropdown
                        ...
                        isMulti={!(widgetInfo?.description === 'GAUGE' || widgetInfo?.description === 'CARD')}
                        closeMenuOnSelect={(widgetInfo?.description === 'GAUGE' || widgetInfo?.description === 'CARD')}
                        isWrappedArray
                        ...
                        defaultValue={deviceSelectData?.find(
                          item => getValues('device')?.filter(data => data === item.value),
                        )}
                      />
                      <p className="text-body-sm text-primary-400">
                  ...
                  <div className="flex justify-between space-x-3">
                    <TitleBar
                      ...
                      className="w-full rounded-md bg-secondary-700 pl-3"
                    />
                    {!(widgetInfo?.description === 'GAUGE' || widgetInfo?.description === 'CARD') ? (
                      <Button
